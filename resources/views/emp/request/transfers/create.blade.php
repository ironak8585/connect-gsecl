...
@endphp
<x-emp-layout pageTitle="Add Transfer Request" :breadcrumbs="$links">
    <x-show.buttons>
        <x-show.back-button />
    ...
    <section class="section">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                <x-show.field label="Designation" value="{{ $employee->designation->name }}" />
            </div>
            <div class="col-md-6">
                <x-show.field label="Current Location" value="{{ $employee->location->name }}" />
            </div>
            <div class="col-md-6">
                <x-show.field label="Department" value="{{ $employee->department->name }}" />
            </div>
        </div>
        <hr>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    <x-form.select-input label="First Preference" name="location_1" :options="$locations"
                        error="{{ $errors->first('location_1') }}" isDummyRequired="true" :required="true" />
                </div>
                <div class="col-md-4">
                    <x-form.select-input label="Second Preference" name="location_2" :options="$locations"
                        error="{{ $errors->first('location_2') }}" isDummyRequired="true" />
                </div>
                <div class="col-md-4">
                    <x-form.select-input label="Third Preference" name="location_3" :options="$locations"
                        error="{{ $errors->first('location_3') }}" isDummyRequired="true" />
                </div>
                <div class="col-md-6">
