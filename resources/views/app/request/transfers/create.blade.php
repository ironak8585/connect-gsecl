@php
    $links = [
        'Home' => '/',
        'transfers' => '/app/request/transfers',
        'Add' => '#'
    ];
@endphp
<x-app-layout pageTitle="Add Transfer Request" :breadcrumbs="$links">
    <x-show.buttons>
        <x-show.back-button />
        <x-show.list-button route="app.request.transfers" can="app_request_transfer_read" label="Transfer Requests" />
    </x-show.buttons>

    <section class="section">
        <form class="row g-3" action="{{ route('app.request.transfers.store') }}" method="POST"
            ...
            <div class="col-md-4">
                <x-form.text label="Circular No." name="transfer_number" error="{{ $errors->first('transfer_number') }}"
                    :required="true" />
            </div>
            <div class="col-md-6">
